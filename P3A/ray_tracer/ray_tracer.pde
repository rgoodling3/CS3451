...
void draw_scene() {
  ...
  for(int y = 0; y < height; y++) {
    for(int x = 0; x < width; x++) {
      ...
      Shape hitobj = null;
      for(Shape s : shapes) {
        //System.out.println(s.checkInter(r));
        if (s.checkInter(r) < mint && s.checkInter(r) > 0) {
          mint = s.checkInter(r);
          hitobj = s;
        }
      }
      ...
      if (hitobj == null || hitobj.getSurface() == null) {
        ...
        PVector normal = hitobj.getNormal(hitpos);
        for (Light l:lights) {
          PVector lightpos = (PVector.sub(l.getlightpos(), hitpos)).normalize();
          pxcolor[0] += l.getRGB()[0] * max(0, PVector.dot(normal,lightpos));
          pxcolor[1] += l.getRGB()[1] * max(0, PVector.dot(normal,lightpos));
          pxcolor[2] += l.getRGB()[2] * max(0, PVector.dot(normal,lightpos));
        }
        pxcolor[0] += hitobj.getSurface().getCar(); //* hitobj.getSurface().getCdr();
        pxcolor[1] += hitobj.getSurface().getCag(); //* hitobj.getSurface().getCdg();
        pxcolor[2] += hitobj.getSurface().getCab(); //* hitobj.getSurface().getCdb();
        
        pxcolor[0] *= hitobj.getSurface().getCdr();
        pxcolor[1] *= hitobj.getSurface().getCdg();
        pxcolor[2] *= hitobj.getSurface().getCdb();
      }
      //set the pixel color
